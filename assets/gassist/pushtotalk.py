    ...
    with SampleAssistant(lang, device_model_id, device_id,
        ...
        class MyHandler(FileSystemEventHandler):
            def on_modified(self, event):
                if "SMARTDISPLAYPI_DID_CALL_GOOGLE_ASSISTANT.yourmother" in event.src_path:
                    print('[DEV]: FILE CHANGED')
                    file = open(event.src_path, "r").read()
                    if "0" in file:
                        print("[DEV]: 0 FOUND: NOT STARTING GOOGLE ASSISTANT")
                    if "1" in file:
                        print("[DEV]: 1 FOUND: STARTING GOOGLE ASSISTANT")
                        assistant.assist()
        ...
        observer = Observer()
        observer.schedule(event_handler, path='/home/' + os.getlogin() + '/SmartDisplayPi/assets/gassist', recursive=False)
        observer.start()
